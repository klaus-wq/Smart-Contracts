...
contract listOfTasks {
    ...
    function setTask(string _title, uint32 _timestamp, bool _isDone) public returns(string, uint32, bool){
        tvm.accept();
        //Task newTask = Task(_title, _timestamp, _isDone);
        //tasks[uint8(keys.length)] = newTask;
        tasks[uint8(keys.length)].title = _title;
        tasks[uint8(keys.length)].timestamp = _timestamp;
        tasks[uint8(keys.length)].isDone = _isDone;
        keys.push(uint8(keys.length));
        return (tasks[uint8(keys.length-1)].title, tasks[uint8(keys.length-1)].timestamp, tasks[uint8(keys.length)].isDone);
    }
    ...
    function getTask(uint8 key) public returns(string, uint32, bool) {
        tvm.accept();
        return (tasks[key].title, tasks[key].timestamp, tasks[key].isDone);
    }
